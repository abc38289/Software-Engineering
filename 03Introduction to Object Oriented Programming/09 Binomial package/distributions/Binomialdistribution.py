...
class Binomial(Distribution):
    ...
    def __init__(self, prob=.5, size=20):
        ...
        self.p = prob # Store probability
        self.n = size # Store number of trials

        # Calculate mean and standard deviation using separate methods
        self.mean = self.calculate_mean()
        self.stdev = self.calculate_stdev()

        # Initialize Distribution class attributes (assuming it has mean and stdev)
    ...
    def plot_bar(self):
        ...
        # Count occurrences of 0 and 1 in the data
        n_success = self.data.count(1)
        n_failure = self.data.count(0)

        # Prepare data for the bar chart
        outcomes = [0, 1]
        counts = [n_failure, n_success]
        ...
        plt.ylabel("Count")
        plt.title("Bar Chart of Binomial Distribution Data")
        plt.show()
    ...
    def pdf(self, k):
        ...
        # Check for invalid input (k outside the range of 0 to n)
        if k < 0 or k > self.n:
          return 0

        # Calculate the PDF using the binomial coefficient and p and q values
        p = self.p
        q = 1 - self.p
        return math.comb(self.n, k) * p**k * q**(self.n - k)

        """ Answer version
        numerator = math.factorial(self.n) / (math.factorial(k) * math.factorial(self.n - k))  # Binomial coefficient
        denominator = p**k * q**(self.n - k)
    ...
    def plot_bar_pdf(self):
        ...
        # Calculate and store PDF values from k=0 to k=n
        for k in range(self.n + 1):
            pdf_value = self.pdf(k)
            x.append(k)
            y.append(pdf_value)
